...
def _llm(flags: list[str], prompt: str, input_text: Optional[str] = None) -> str:
    ...
    try:
        ...
        typer.secho("❌ LLM command failed.", fg=typer.colors.RED)
        return ""
...
def _read_from_clipboard() -> str:
    ...
    try:
        ...
        typer.secho("⚠️ Failed to read from clipboard.", fg=typer.colors.YELLOW)
        return ""
            ...
            try:
                if p.stat().st_mtime < cutoff_time:
                    ...
                    try:
                        ...
                        typer.secho(f"⚠️ Failed to delete file: {p}", fg=typer.colors.YELLOW)
...
def _git_main_branch() -> Optional[str]:
    ...
    try:
        ...
        typer.secho("⚠️ Unable to determine main branch name.", fg=typer.colors.YELLOW)
        return None
    return None
...
def _git_merge_base(branch: Optional[str] = None) -> Optional[str]:
    ...
    try:
        ...
        typer.secho("⚠️ Unable to determine merge-base.", fg=typer.colors.YELLOW)
        return None
        ...
        if len(parts) >= 2:
            ...
            for p in merged_parents:
                try:
                    ...
                    typer.secho("⚠️ Error checking ancestor status.", fg=typer.colors.YELLOW)
...
def _read_local_template(filename: str) -> Optional[str]:
    ...
    try:
        ...
        typer.secho(f"⚠️ Unable to read local template file: {filename}", fg=typer.colors.YELLOW)
        pass
    return None
...
def _get_first_commit(start_hash: str, pattern: str, match: bool) -> CommitResult:
    ...
    try:
        ...
        typer.secho("⚠️ Error occurred while searching for commit; returning None.", fg=typer.colors.YELLOW)
...
def _truncate_hash(sha: str) -> str:
    ...
    try:
        ...
        typer.secho("⚠️ Unable to determine short hash length; using default.", fg=typer.colors.YELLOW)
...
def _get_start_hash(pattern: str = "UNPICK", match: bool = False, repo: Optional[str] = None) -> str:
    ...
    try:
        ...
        if not start_sha_full:
            typer.secho("⚠️ Could not resolve start SHA for {START}; leaving empty.", fg=typer.colors.YELLOW)
            return ""

        return _truncate_hash(start_sha_full)
    except Exception:
        typer.secho("⚠️ Error occurred while resolving {START}; leaving empty.", fg=typer.colors.YELLOW)
        return ""
...
def _git_diff_text(items: List[str]) -> str:
    ...
    try:
        ...
        typer.secho("⚠️ Error running git diff command.", fg=typer.colors.YELLOW)
        return ""
...
def clipboard_to_file(
    ...
    try:
        ...
        typer.secho("⚠️ Could not generate short title from clipboard; using fallback.", fg=typer.colors.YELLOW)
        pass
...
def commits_between(
    ...
    def _run_log(rng: str) -> List[str]:
        try:
            ...
            typer.secho("⚠️ Error running git log command.", fg=typer.colors.YELLOW)
            return []
...
def gsm() -> None:
    ...
    try:
        ...
        try:
            ...
            typer.secho(f"❌ Additionally, failed to return to {cur_branch}.", fg=typer.colors.RED)
            pass
        raise typer.Exit(1)
...
def gsquash(
    ...
    if _run(["git", "merge-base", "--is-ancestor", c2, f"refs/heads/{target_branch}"], check=False).returncode == 0:
        ...
        try:
            ...
            typer.secho("⚠️ Cherry-pick encountered conflicts.", fg=typer.colors.YELLOW)
...
def gggrbi(args: List[str] = typer.Argument(None)) -> None:
    ...
    try:
        ...
        typer.secho("❌ Failed to read git config.", fg=typer.colors.RED)
        pass
...
def gcommit_cmd(message: Optional[str] = typer.Argument(None, help="Commit message; if omitted, generate via llm")) -> None:
    ...
    if not msg:
        ...
        try:
            ...
            typer.secho("❌ Not a git repo or failed to get staged diff.", fg=typer.colors.RED)
            staged = ""
...
def gtest(commit_point: Optional[str] = typer.Argument(None, help="Commit point (default: HEAD)")) -> None:
    ...
    if current_branch == "test":
        ...
        try:
            ...
            typer.secho("⚠️ Failed to switch from 'test' branch. Please switch manually and rerun.", fg=typer.colors.RED)
            pass
...
def _get_template_from_clipboard_or_stdin(prefix: str, pr_number: str) -> str:
    ...
    if _is_macos_with_pbcopy():  # pbcopy implies pbpaste availability
        ...
        if text:
            ...
            try:
                p = Path.home() / "tmp" / f"{pr_number}-{prefix}.md"
            ...
            try:
                ...
                typer.secho("❌ Failed to read from stdin.", fg=typer.colors.RED)
                text = ""
    else:
...
def swapmsgs(
    ...
    try:
        ...
        try:
            ...
            typer.secho(f"❌ Failed to checkout original branch {cur_branch}. Please switch manually.", fg=typer.colors.RED)
            pass
...
def _handle_apply_failure(proc: subprocess.CompletedProcess, patch_path: Path) -> None:
    ...
    try:
        ...
        typer.secho("⚠️ Failed to determine git repository root; assuming current directory.", fg=typer.colors.YELLOW)
        root = "."
    ...
    try:
        ...
        typer.secho("⚠️ Failed to get git status for debugging.", fg=typer.colors.YELLOW)
...
def _read_clipboard_content() -> str:
    ...
    try:
        ...
        typer.secho("⚠️ Failed to run pbpaste.", fg=typer.colors.YELLOW)
        clip = ""
...
def grevdiff() -> None:
    ...
    try:
        ...
        try:
            ...
            typer.secho("⚠️ Failed to remove rev.patch; please delete manually.", fg=typer.colors.YELLOW)
            pass
        raise typer.Exit(1)
...
def gfilecommit(file: str = typer.Argument(..., help="File path to stage and commit")) -> None:
    ...
    try:
        ...
        typer.secho("❌ Failed to get staged diff.", fg=typer.colors.RED)
        staged_diff = ""
...
def gsign(
    ...
    try:
        ...
        typer.secho("❌ Failed to determine .git directory.", fg=typer.colors.RED)
        git_dir = ""
    ...
    try:
        ...
        typer.secho("⚠️ Failed to determine current branch; assuming detached HEAD.", fg=typer.colors.YELLOW)
        cur_branch = "HEAD"
...
def cdiff_cmd() -> None:
    ...
    try:
        ...
        try:
            ...
            typer.secho("⚠️ Failed to remove temp files; please delete manually.", fg=typer.colors.YELLOW)
            pass
...
def ideep(
    ...
    try:
        ...
        typer.secho("❌ Failed to generate concise summary.", fg=typer.colors.RED)
        pass
...
def iprfb(
    ...
    if _which("llm"):
        try:
            ...
            typer.secho("⚠️ Failed to generate short title using llm; using fallback.", fg=typer.colors.YELLOW)
            pass
    ...
    if comment.strip():
        if _which("llm"):
            try:
                ...
                typer.secho("⚠️ Failed to rephrase comment using llm; using original.", fg=typer.colors.YELLOW)
                rephrased = comment
        else:
            rephrased = comment
...
def cleantmp_cmd(
    ...
    if pattern is not None and not isinstance(pattern, str):
        try:
            ...
            typer.secho("⚠️  Unexpected pattern type; treating as no pattern.", fg=typer.colors.YELLOW)
...
def _run_prefixed_cleanup_and_notify(prefixes: List[str] = ["gdiff-", "gdn-", "gs-", "ctest-", "crun-", "maturin-"]) -> None:
    ...
    try:
        ...
        typer.secho("⚠️  Failed to build regex pattern from prefixes; using fallback pattern.", fg=typer.colors.YELLOW)
...
def _notify_macos(message: str, title: str = "Notification") -> None:
    ...
    try:
        ...
        typer.secho("⚠️  Failed to show macOS notification.", fg=typer.colors.YELLOW)
...
def _stamp_on_success(stamp_file: Path, epoch: int):
    ...
    try:
        ...
        try:
            ...
            typer.secho(f"⚠️  Could not write stamp file {stamp_file}", fg=typer.colors.YELLOW)
